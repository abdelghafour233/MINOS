...
const App: React.FC = () => {
  ...
  // تحميل الطلبات من ذاكرة المتصفح المحلية عند بدء التشغيل
  useEffect(() => {
  ...
  return (
    <div className={`min-h-screen ${selectedProduct || showLogin ? 'no-scroll' : ''}`}>
      ...
      {/* شريط التنقل السفلي للهواتف والجانبي للحواسيب */}
        ...
        {view === 'shop' ? (
          <div className="space-y-12 animate-in">
            ...
            <header className="relative h-56 md:h-80 rounded-[2.5rem] overflow-hidden flex items-center px-8 md:px-16 border border-white/10 shadow-2xl">
              ...
              <div className="relative z-20 space-y-4 text-right">
                <span className="bg-emerald-500/10 text-emerald-400 px-4 py-1.5 rounded-full text-xs font-black border border-emerald-500/20 uppercase tracking-widest">المغرب 🇲🇦</span>
                <h1 className="text-4xl md:text-6xl font-black text-grad leading-tight">بريمة ستور<br/>جودة تستحقها</h1>
                <p className="text-slate-400 font-bold max-w-md">أفضل المنتجات العصرية بين يديك مع خدمة توصيل سريعة ومجانية في كل المدن المغربية.</p>
              </div>
            </header>
              ...
              {products.map(p => (
                <div key={p.id} onClick={() => setSelectedProduct(p)} className="glass rounded-[2rem] overflow-hidden cursor-pointer hover:border-emerald-500/40 transition-all border border-white/10 group">
                  <div className="aspect-square bg-slate-900 overflow-hidden relative">
                    ...
                    <div className="absolute top-4 right-4 bg-emerald-500 text-black px-3 py-1 rounded-full text-[10px] font-black uppercase shadow-lg">-50% OFF</div>
                  </div>
                  <div className="p-5 text-right space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="bg-white/5 p-2 rounded-xl text-emerald-500 group-hover:bg-emerald-500 group-hover:text-black transition-colors"><ShoppingCart size={18}/></div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-8 animate-in text-right">
            ...
            <div className="space-y-4">
              {orders.length === 0 ? (
                <div className="py-24 text-center glass rounded-[2.5rem] text-slate-500 font-bold border-2 border-dashed border-white/10">
                  قائمة الطلبات فارغة حالياً
                </div>
              ) : (
                orders.map(o => (
                  <div key={o.orderId} className="glass p-6 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-6 border border-white/5 hover:border-emerald-500/20 transition-all group">
                    <div className="text-right flex-1 space-y-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-black bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full">{o.orderId}</span>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex gap-3">
                        <a 
                          href={`https://wa.me/212${o.customer.phoneNumber.replace(/^0/, '')}`} 
                          target="_blank" 
                          rel="noreferrer" 
                          className="p-4 bg-emerald-500/10 text-emerald-500 rounded-2xl hover:bg-emerald-500 hover:text-black transition-all shadow-lg border border-emerald-500/20"
                        >
        ...
        <div className="fixed inset-0 z-[5000] flex items-center justify-center p-6 bg-slate-950/95 backdrop-blur-xl">
          <div className="max-w-xs w-full glass p-10 rounded-[3rem] text-center space-y-8 border border-white/20 shadow-2xl">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 font-bold">يرجى إدخال كلمة السر الخاصة بالمدير</p>
            </div>
            <input 
              type="password" 
              placeholder="••••" 
              ...
              onKeyPress={(e) => e.key === 'Enter' && (pass === 'admin' ? (setIsAdmin(true), setShowLogin(false), setView('admin'), setPass('')) : notify('كلمة السر خاطئة'))}
            />
            <button 
            ...
            <button onClick={() => setShowLogin(false)} className="text-slate-500 text-sm font-bold hover:text-white">العودة للمتجر</button>
          </div>
        </div>
      )}

      {/* نافذة عرض المنتج والطلب */}
      {selectedProduct && (
        <div className="fixed inset-0 z-[4000] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-5xl glass rounded-[3rem] overflow-hidden flex flex-col md:flex-row animate-in border border-white/20 max-h-[92vh] shadow-2xl">
            <button 
              onClick={() => { setSelectedProduct(null); setIsCheckout(false); }} 
              className="absolute top-6 right-6 z-50 p-3 bg-black/60 rounded-full text-white hover:bg-rose-500 transition-all border border-white/10"
            >
              <X size={20}/>
            </button>
            
            <div className="w-full md:w-1/2 bg-slate-900/50 flex items-center justify-center p-12 overflow-hidden">
              <img 
                src={selectedProduct.thumbnail} 
                className="max-h-64 md:max-h-full object-contain drop-shadow-2xl" 
              ...
              {!isCheckout ? (
                <div className="space-y-8 flex flex-col h-full">
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-end gap-1.5 text-amber-500">
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div className="flex gap-4 items-center">
                      <div className="flex items-center gap-2 bg-emerald-500/10 text-emerald-400 px-4 py-2 rounded-2xl border border-emerald-500/10">
                        <Check size={16}/> <span className="text-xs font-bold">متوفر في المخزون</span>
                      </div>
                      <div className="flex items-center gap-2 bg-white/5 text-slate-400 px-4 py-2 rounded-2xl">
                        <Phone size={16}/> <span className="text-xs font-bold">دعم 24/7</span>
                      </div>
                    </div>
                  </div>
                  <div className="pt-8 border-t border-white/10">
                    <div className="flex justify-between items-end mb-8">
                      <div>
                        <p className="text-4xl md:text-5xl font-black text-white">{selectedProduct.price} <span className="text-sm font-black text-emerald-500">{STORE_CONFIG.currency}</span></p>
                        <p className="text-[10px] text-emerald-500 font-black uppercase tracking-widest mt-1">شاملة لمصاريف التوصيل 📦</p>
                      </div>
                      <div className="text-slate-500 line-through font-bold text-xl">{selectedProduct.price + 100} {STORE_CONFIG.currency}</div>
                    </div>
                    <button 
                      onClick={() => setIsCheckout(true)} 
                      className="w-full bg-emerald-500 text-black py-5.5 rounded-2xl font-black text-xl shadow-2xl shadow-emerald-500/30 hover:bg-emerald-400 transition-all transform active:scale-95"
                    >
                ...
                <div className="space-y-8 py-4 animate-in">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-400 font-bold">سنتصل بك فوراً بعد تأكيد طلبك لتحديد موعد التوصيل.</p>
                  </div>
                  ...
                  <div className="pt-6 space-y-4">
                    <button 
                      onClick={handleOrder} 
                      className="w-full bg-emerald-500 text-black py-5.5 rounded-2xl font-black text-xl shadow-xl hover:bg-emerald-400 transition-all border border-emerald-500/20"
                    >
                      تأكيد الطلب بنجاح
                    </button>
                    <button 
                      onClick={() => setIsCheckout(false)} 
                      className="w-full text-slate-500 text-sm font-bold hover:text-white"
                    >
      ...
      {success && (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center p-6 bg-slate-950/98 backdrop-blur-3xl">
          <div className="max-w-md w-full glass p-14 rounded-[4rem] text-center space-y-10 border border-emerald-500/30 shadow-2xl animate-in">
            <div className="w-28 h-28 bg-emerald-500 rounded-full flex items-center justify-center mx-auto text-black shadow-2xl animate-bounce">
              <Check size={56} strokeWidth={4}/>
            </div>
            <div className="space-y-4">
                <h3 className="text-4xl font-black text-white">تم استلام طلبك!</h3>
                <p className="text-slate-400 font-bold text-lg leading-relaxed">شكراً لثقتك بمتجر بريمة. سنتصل بك قريباً عبر الهاتف لتأكيد العنوان وشحن المنتج.</p>
            </div>
            <button 
              onClick={() => setSuccess(false)} 
              className="w-full bg-emerald-500 text-black py-5.5 rounded-[2rem] font-black text-xl shadow-lg hover:bg-emerald-400 transition-all"
            >
              حسناً، فهمت
            </button>
          </div>
