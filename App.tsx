                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="glass-morphism p-10 rounded-[3rem] border border-white/5 space-y-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2"><label className="text-xs font-black text-slate-500 px-2">Anon Key</label><input type="password" className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl font-bold focus:border-emerald-500 outline-none" value={dbConfig.key} onChange={e => setDbConfig({...dbConfig, key: e.target.value})} placeholder="المفتاح السري (anon)" /></div>
                  ...
                  <div key={order.orderId} className="glass-morphism p-8 rounded-[2.5rem] flex flex-col md:flex-row md:items-center justify-between gap-6 border border-white/5 shadow-xl">
                    ...
                    <div className="flex flex-col md:items-end gap-3 text-right">
                       ...
                       <div className="flex items-center gap-2">
                         <button onClick={() => updateOrderStatus(order.id, 'pending')} className={`px-3 py-1 rounded-lg text-[10px] font-black ${order.status === 'pending' ? 'bg-amber-500 text-black' : 'bg-white/5 text-slate-500'}`}>قيد المعالجة</button>
                         <button onClick={() => updateOrderStatus(order.id, 'shipped')} className={`px-3 py-1 rounded-lg text-[10px] font-black ${order.status === 'shipped' ? 'bg-blue-500 text-white' : 'bg-white/5 text-slate-500'}`}>تم الشحن</button>
                         <button onClick={() => updateOrderStatus(order.id, 'delivered')} className={`px-3 py-1 rounded-lg text-[10px] font-black ${order.status === 'delivered' ? 'bg-emerald-500 text-black' : 'bg-white/5 text-slate-500'}`}>تم التوصيل</button>
                       </div>
                    </div>
                  </div>
                )) : (
